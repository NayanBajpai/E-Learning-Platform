...
redirect("index.php");
  ...
  $sql = "SELECT * FROM exercise WHERE LessonID = '{$id}'";
  $mydb->setQuery($sql);
  $cur = $mydb->loadResultList();

  foreach ($cur as $res) {
  	# code...
  	$sql = "SELECT * FROM score s,exercise e WHERE s.ExerciseID=e.ExerciseID AND e.ExerciseID='{$res->ExerciseID}' and StudentID='{$studentid}'";
  	$mydb->setQuery($sql);
